...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'nopic.gif') {
	...
	if($width>=$width_t || $height>=$height_t){
		if($width/$height>$width_t/$height_t){
			$height=round($width_t*$height/$width);	
			$width=$width_t;			
		}else{
			$width=round($height_t*$width/$height);
			$height=$height_t;
		}
	}
...
function seo($siteid, $catid = '', $title = '', $description = '', $keyword = '') {
	...
	$seo['site_title'] =isset($site['site_title']) && !empty($site['site_title']) ? $site['site_title']." - ".$site['name'] : $site['name'];
	...
	$seo['title'] =  (isset($title) && !empty($title) ? $title.' - ' : '').(isset($cat['catname']) && !empty($cat['catname']) ? $cat['catname'].' - '.$site['name']: '');
	$seo['name'] = $site['name'];
	foreach ($seo as $k=>$v) {
